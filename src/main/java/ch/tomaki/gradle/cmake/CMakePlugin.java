...
public class CMakePlugin implements Plugin<Project> {
  ...
  private void afterEvaluate(final Project project) {
    try {
      ...
      if (Objects.nonNull(extension)) {
        ...
        final CMakeResolver resolver = new CMakeResolver(extension.getToolchains(), extension.getPackages(),
            project);
        ...
        extension.getCustomTasks().forEach((taskName, toolchainNames) -> {
          toolchainNames.forEach((toolchainName) -> {
            Optional.ofNullable(toolchains.get(taskName)).ifPresent((toolchain) -> {
              configureCustomTask(taskRegistry, taskName, toolchain);
            });
          });
        });

        configureAssembleListsTask(taskRegistry, toolchains.values(), project);
        for (final CMakeResolvedToolchain toolchain : toolchains.values()) {
  ...
  private void configureAssembleConfigTask(final CMakeTaskRegistry taskRegistry, final CMakeResolvedToolchain toolchain,
    ...
    final String cmakeToolchainBuildAllTaskName = CMakeTasksConventions.buildAllTaskName(toolchain
        .getName());
    taskRegistry.register(cmakeToolchainBuildAllTaskName)
        .configure((task) -> task.setGroup(CMakeTasksConventions.GROUP_BUILD));

    final String cmakeToolchainCheckAllTaskName = CMakeTasksConventions.checkAllTaskName(toolchain
        .getName());
