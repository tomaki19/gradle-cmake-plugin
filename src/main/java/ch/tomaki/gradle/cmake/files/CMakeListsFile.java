...
public class CMakeListsFile extends CMakeFileOutputStream {
  ...
  private void writeHeader(final Project project)
      throws IOException {
  ...
  private void writeLibraries(final Set<CMakeResolvedLibrary> libraries, final Project project) throws IOException {
  ...
  private void writeTests(final Set<CMakeResolvedTest> tests, final Project project) throws IOException {
    if (!tests.isEmpty()) {
      writeLine();
  ...
  private void writeInterfaceLibrary(final CMakeResolvedLibrary library, final Project project) throws IOException {
    final String target = CMakeListsConventions.interfaceLibraryTarget(library.getName(), library.getToolchain(),
  ...
  private void writeStaticLibrary(final CMakeResolvedLibrary library, final Project project) throws IOException {
    final String target = CMakeListsConventions.staticLibraryTarget(library.getName(), library.getToolchain(),
        library.getBuildConfig());
  ...
  private void writeSharedLibrary(final CMakeResolvedLibrary library, final Project project) throws IOException {
  ...
  private void writePrivateLinking(final String target, final CMakeResolvedBinary binary)
    ...
    if (!binary.getPrivateFindPackageDependencies().isEmpty()
      ...
      writeTargetLinkLibraries(target, "PRIVATE", binary.getPrivateProjectModuleDependencies(),
          binary.getPrivateFindPackageDependencies(), binary.getPrivateLinkOptions());
    }
  }

  private void writePublicLinking(final String target, final CMakeResolvedLibrary library) throws IOException {
    if (!library.getPublicFindPackageDependencies().isEmpty()
      ...
      writeTargetLinkLibraries(target, "PUBLIC", library.getPublicProjectModuleDependencies(),
          library.getPublicFindPackageDependencies(), library.getPublicLinkOptions());
    }
  }
  ...
  private void writeOutputTargetProperties(final String target, final CMakeResolvedToolchain toolchain,
    ...
    for (final String buildConfig : toolchain.getBuildConfigs()) {
      write(1, "ARCHIVE_OUTPUT_DIRECTORY_%s \"%s\"", buildConfig.toUpperCase(),
          installDir.toURI().getPath());
    }
    write(1, "ARCHIVE_OUTPUT_DIRECTORY \"%s\"", installDir.toURI().getPath());
    for (final String buildConfig : toolchain.getBuildConfigs()) {
      write(1, "LIBRARY_OUTPUT_DIRECTORY_%s \"%s\"", buildConfig.toUpperCase(),
          installDir.toURI().getPath());
    }
    write(1, "LIBRARY_OUTPUT_DIRECTORY \"%s\"", installDir.toURI().getPath());
    for (final String buildConfig : toolchain.getBuildConfigs()) {
      write(1, "RUNTIME_OUTPUT_DIRECTORY_%s \"%s\"", buildConfig.toUpperCase(),
          installDir.toURI().getPath());
    }
  ...
  private void writeStripDebugCommand(final String target, final CMakeResolvedToolchain toolchain,
    ...
    write(1, "COMMAND ${CMAKE_COMMAND} -E copy $<TARGET_FILE:%s> %s/%s.debug", target, installDir.toURI().getPath(),
        target);
