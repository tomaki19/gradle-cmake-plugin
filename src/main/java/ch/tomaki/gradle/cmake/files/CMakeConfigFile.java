...
public class CMakeConfigFile extends CMakeFileOutputStream {
  ...
  public void write(final CMakeResolvedBuild build, final Project project) throws IOException {
    for (final CMakeResolvedLibrary library : build.getLibraries()) {
      if (library.getSources().isEmpty()) {
        final String libraryTarget = CMakeListsConventions.interfaceLibraryTarget(library.getName(),
        ...
        if (library.isBuildStatic()) {
          final String libraryTarget = CMakeListsConventions.staticLibraryTarget(library.getName(),
              library.getToolchain(), library.getBuildConfig());
          write("add_library( %s::%s STATIC IMPORTED )", project.getName(), libraryTarget);
          setTargetProperties(library, libraryTarget, OperatingSystem.current().getStaticLibrarySuffix(), project);
        }
        if (library.isBuildShared()) {
          final String libraryTarget = CMakeListsConventions.sharedLibraryTarget(library.getName(),
              library.getToolchain(), library.getBuildConfig());
          write("add_library( %s::%s SHARED IMPORTED )", project.getName(), libraryTarget);
          setTargetProperties(library, libraryTarget, OperatingSystem.current().getSharedLibrarySuffix(), project);
        }
      }
    }
  }
  ...
  private void setTargetProperties(final CMakeResolvedLibrary library, final String libraryTarget,
    ...
    final File installDir = project.getLayout().getBuildDirectory().get()
        .dir("%s/%s".formatted(CMakeListsConventions.CMAKE_INSTALL_PATH, library.getToolchain().getName())).getAsFile();
