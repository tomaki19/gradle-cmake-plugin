...
public class CMakeConfigFile extends CMakeFileContent {
  ...
  public void writeTo(final FileOutputStream outputStream) throws IOException {
    ...
    writeSystemPackageDependencies(outputStream, toolchain.getPackages());
  ...
  private void writeSystemPackageDependencies(final FileOutputStream outputStream,
  ...
  private void setTargetProperties(final FileOutputStream outputStream, final CMakeResolvedLibrary library,
    ...
    final File workingDir = getBuildDirectory().dir(CMakeFileConventions.CMAKE_EXPORT_PATH).getAsFile();
  ...
  private void setTargetProperties(final FileOutputStream outputStream, final CMakeResolvedLibrary library,
    ...
    final Directory installDir = getBuildDirectory()
        .dir("%s/%s/%s".formatted(CMakeFileConventions.CMAKE_EXPORT_PATH, toolchain.getName(), buildConfig));
    ...
    write(outputStream, 2, "IMPORTED_LOCATION_%s \"${CMAKE_CURRENT_LIST_DIR}/%s/%s\"",
        buildConfig.toString().toUpperCase(),
